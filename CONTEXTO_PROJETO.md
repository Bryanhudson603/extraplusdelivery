...
**Próximo passo:** o cliente final (Ewerton) precisa: (1) baixar a versão nova de `windows-print-bridge/server.js` (arquivo local, não é deploy automático) e reiniciar o bridge; (2) aguardar o deploy do frontend na Vercel; (3) testar uma impressão real; (4) se ainda sair errado, usar o novo campo "Caracteres por linha do cupom" no painel de Pedidos pra ajustar ao vivo, sem precisar de outro fix de código.

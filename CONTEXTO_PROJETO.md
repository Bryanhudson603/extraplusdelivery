...
**Próximo passo:** o usuário precisa baixar a versão nova e rodar `instalar-inicializacao-automatica.ps1` na máquina do admin (dentro da pasta `windows-print-bridge`), e confirmar que (a) a tarefa aparece no Agendador de Tarefas como "Extraplus Windows Print Bridge", (b) o bridge conecta sozinho sem terminal visível, e (c) a impressão automática continua funcionando ao chegar pedido novo.

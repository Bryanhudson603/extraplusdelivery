...
**Próximo passo:** o cliente final (Ewerton) precisa: (1) atualizar/baixar a versão nova de `print-bridge.ts` (deploy do frontend na Vercel, nada a reinstalar no Windows) e confirmar que "Sincronizar bridge" conecta sem erro de CORS; (2) testar uma impressão real de pedido e confirmar visualmente que o cupom agora sai formatado corretamente na bobina de 57mm, sem quebra por caractere — se a impressora dele usar uma fonte ESC/POS diferente e o texto ficar apertado ou sobrar muito espaço, ajustar `RECEIPT_WIDTH_CHARS` em `admin/orders/page.tsx`.

...
**Próximo passo:** o cliente final (Ewerton) precisa atualizar/baixar a versão nova de `print-bridge.ts` (via novo deploy do frontend na Vercel — esse arquivo é frontend, não precisa reinstalar nada na máquina Windows dele) e confirmar que "Sincronizar bridge" no painel conecta sem erro de CORS, e que a impressão automática funciona ao chegar pedido novo.
